<x-app-layout>
    ...
    <div class="my-4 text-center text-2xl text-indigo-600 font-extrabold">Markup Rates - {{$client->name}}</div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        @foreach($markups as $markup)
                            <tr class="{{$loop->iteration % 2 == 0 ? 'bg-white' : 'bg-gray-100'}} ">
                                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{ $markup->markup }}</td>
                                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{$markup->valid_from->format('d-m-Y')}}</td>
                                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{isset($markup->valid_to) ? $markup->valid_to->format('d-m-Y') : 'Ongoing'}}</td>
                            </tr>
                        @endforeach
                ...
                <form method="Post" action="{{route('markup.store')}}">
                    ...
                    <div class="max-w-5xl mx-auto">
                        <div class="row flex justify-end mt-8">
                            <a href="{{route('markup')}}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">Back</a>
                        </div>
                    </div>
                </form>
            </div>
